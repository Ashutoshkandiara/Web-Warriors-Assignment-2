...
function createLightbox() {
    ...
    function createOverlay() {
        ...
        //Add a button to the figure box
        let actionButton = document.createElement("button");
        actionButton.className = "actionButton";
        actionButton.textContent = "Add to favourites";
        actionButton.onclick = function() {
            addToFavourites(overlayImage);
        };
        figureBox.appendChild(actionButton);
    ...
    function addToFavourites(image) {
        ...
        if (selectionBox.childElementCount < 5) {
            let favContainer = document.createElement("div");
            favContainer.className = "favContainer";

            let favImage = image.cloneNode(true);
            favImage.onclick = createFavOverlay;
            favContainer.appendChild(favImage);

            selectionBox.appendChild(favContainer);
        } else {
            alert("You can only add up to 5 favourites. Press remove button to remove images from the favourites.");
        }
    }
    
    function createFavOverlay() {
        let overlay = document.createElement("div");
        ...
        removeButton.onclick = () => {
            let selectionBox = document.getElementById("selectionbox");
            selectionBox.removeChild(this.parentElement);
            document.body.removeChild(overlay);
        };
        figureBox.appendChild(removeButton);
        
        //Add a close button to the overlay
        let closeBox = document.createElement("div");
        closeBox.id = "favOverlayClose";
        closeBox.innerHTML = "&times;";
        closeBox.onclick = function() {
            document.body.removeChild(overlay);
        };
